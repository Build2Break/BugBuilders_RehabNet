...
# Choose target image based on your existing logic
bike = 3
...
# ==============================
# Ask user for how long to record
# ==============================
record_seconds = int(input("Enter recording duration (in seconds): "))

# ==============================
# ADD-ON: Track per-second pose scores
# ==============================
second_pose_scores = []
frame_pose_scores = []
start_time = time.time()
next_second_time = start_time + 1

# ==============================
# Pose detection loop with timer
# ==============================
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    while cap.isOpened():
        ...
        try:
            ...
            # ==============================
            # Your existing pose logic (exactly unchanged)
            # ==============================
            ...
            keypoints = [{'X': p.x, 'Y': p.y, 'Z': p.z} for p in landmarks]
            
            p_score = dif_compare(keypoints, point_target)      
            
            angle = []
            angle.append(int(calculateAngle(right_shoulder, right_elbow, right_wrist)))
            angle.append(int(calculateAngle(left_shoulder, left_elbow, left_wrist)))
            angle.append(int(calculateAngle(right_elbow, right_shoulder, right_hip)))
            angle.append(int(calculateAngle(left_elbow, left_shoulder, left_hip)))
            angle.append(int(calculateAngle(right_shoulder, right_hip, right_knee)))
            angle.append(int(calculateAngle(left_shoulder, left_hip, left_knee)))
            angle.append(int(calculateAngle(right_hip, right_knee, right_ankle)))
            angle.append(int(calculateAngle(left_hip, left_knee, left_ankle)))
            
            compare_pose(image, angle_point, angle, angle_target)
            a_score = diff_compare_angle(angle, angle_target)
            
            display_score = max((1 - a_score), (1 - p_score)) * 100
            
            cv2.putText(image, str(int(display_score)), (80,30),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, [0,0,255], 2, cv2.LINE_AA)

            # ==============================
            # ADD-ON: collect frame’s pose score
            # ==============================
            frame_pose_scores.append(display_score)
        ...
        # ==============================
        # ADD-ON: end of 1-sec bucket logic
        # ==============================
        current_time = time.time()
        if current_time >= next_second_time:
            if frame_pose_scores:
                second_pose_scores.append(sum(frame_pose_scores) / len(frame_pose_scores))
            else:
                second_pose_scores.append(0)
            frame_pose_scores = []
            next_second_time += 1
...
# ==============================
# ADD-ON: Print results
# ==============================
print("\nPose Comparison per second:")
for i, val in enumerate(second_pose_scores):
    print(f"Second {i+1}: {val:.2f}")

if second_pose_scores:
    overall_avg_pose = sum(second_pose_scores) / len(second_pose_scores)
else:
    overall_avg_pose = 0

print(f"\nOverall Average Pose Score: {overall_avg_pose:.2f}")
